...
const SMAAEffectRaw = SMAAEffect as unknown as {
  searchImageDataURL: string;
  areaImageDataURL: string;
};

const SMAAPresetRaw = SMAAPreset as unknown as {
  MEDIUM: number;
};
...
interface ResolvedEffectColors {
  roadColor: number;
  islandColor: number;
  background: number;
  shoulderLines: number;
...
const DEFAULT_EFFECT_OPTIONS: Required<Omit<HyperspeedEffectOptions, "onSpeedUp" | "onSlowDown">> =
  {
    ...
    colors: {
      ...
      background: 0x09090b,
      shoulderLines: 0x18181b,
      brokenLines: 0x27272a,
      leftCars: [0xa78bfa, 0x8b5cf6, 0x6366f1],
      rightCars: [0xffffff, 0xe4e4e7, 0xa78bfa],
      sticks: 0xa78bfa,
    },
  };
...
const pickRandom = <T,>(arr: T[] | T): T => {
  if (Array.isArray(arr)) {
    ...
    if (val === undefined) {
      throw new Error("pickRandom received empty array");
    }
    return val;
  }
  return arr;
...
class App {
  ...
  constructor(container: HTMLDivElement, options: ResolvedEffectOptions) {
    ...
    if (gl) {
      const originalGetContextAttributes = gl.getContextAttributes.bind(gl);
      gl.getContextAttributes = () => {
        try {
          const attrs = originalGetContextAttributes();
  ...
  initPasses() {
    ...
    const smaaPass = new EffectPass(
      this.camera,
      new SMAAEffect({
        preset: SMAAPresetRaw.MEDIUM,
        searchImage: SMAAEffectRaw.searchImageDataURL,
        areaImage: SMAAEffectRaw.areaImageDataURL,
      } as unknown as { preset?: SMAAPreset }),
    );
  ...
  loadAssets() {
    ...
    return new Promise<void>((resolve) => {
      ...
      searchImage.src = SMAAEffectRaw.searchImageDataURL;
      areaImage.src = SMAAEffectRaw.areaImageDataURL;
    });
  }
...
class CarLights {
  ...
  init() {
    ...
    const colorsVal = this.colors;
    let colors: THREE.Color[] | THREE.Color;
    if (Array.isArray(colorsVal)) {
      colors = colorsVal.map((c) => new THREE.Color(c));
    } else {
      colors = new THREE.Color(colorsVal);
    }
...
class LightsSticks {
  ...
  constructor(webgl: App, options: ResolvedEffectOptions) {
    this.webgl = webgl;
    this.options = options;
  }
  ...
  init() {
    ...
    const colorsVal = options.colors.sticks;
    let colors: THREE.Color[] | THREE.Color;
    if (Array.isArray(colorsVal)) {
      colors = colorsVal.map((c) => new THREE.Color(c));
    } else {
...
export const Hyperspeed = ({ effectOptions, className, style }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    const colors: ResolvedEffectColors = {
      roadColor: effectOptions?.colors?.roadColor ?? 0x080808,
      islandColor: effectOptions?.colors?.islandColor ?? 0x0a0a0a,
      background: effectOptions?.colors?.background ?? 0x09090b,
      shoulderLines: effectOptions?.colors?.shoulderLines ?? 0x18181b,
      brokenLines: effectOptions?.colors?.brokenLines ?? 0x27272a,
      leftCars: effectOptions?.colors?.leftCars ?? [0xa78bfa, 0x8b5cf6, 0x6366f1],
      rightCars: effectOptions?.colors?.rightCars ?? [0xffffff, 0xe4e4e7, 0xa78bfa],
      sticks: effectOptions?.colors?.sticks ?? 0xa78bfa,
    };

    const distortionKey = effectOptions?.distortion || DEFAULT_EFFECT_OPTIONS.distortion;
    const distortion = distortions[distortionKey] || distortions[DEFAULT_EFFECT_OPTIONS.distortion];
    if (!distortion) {
      throw new Error(`Distortion ${distortionKey} not found`);
    }

    const options: ResolvedEffectOptions = {
    ...
    try {
      ...
      console.warn("Failed to initialize Hyperspeed WebGL background:", e);
    }
  ...
  return <div id="lights" className={className} style={style} ref={hyperspeed}></div>;
};
