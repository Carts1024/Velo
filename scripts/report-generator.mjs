...
const lines = ["# Benchmark report", "", `Evidence captured: ${runs.map((run) => run.capturedAt).sort()[0] ?? "unknown"}`, "", "| Scenario | Profile | Temperature | Window | Target/achieved arrival | Saturated | Successful throughput | Error rate | p50 | p95 | p99 |", "| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |"];
for (const run of runs) lines.push(`| ${run.scenario} | ${run.profile} | ${run.temperature} | ${run.window} | ${run.pacing.targetRequestsPerSecond}/${run.pacing.achievedRequestsPerSecond} rps | ${run.saturation.saturatedArrivals} | ${run.throughput.successfulPerSecond}/s | ${(run.errorSamples / run.attemptedSamples * 100).toFixed(2)}% | ${run.latencyMs.p50} ms | ${run.latencyMs.p95} ms | ${run.latencyMs.p99} ms |`);
